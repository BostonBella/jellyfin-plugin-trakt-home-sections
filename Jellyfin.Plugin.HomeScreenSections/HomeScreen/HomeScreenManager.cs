...
using Jellyfin.Plugin.TraktHomeSections.HomeScreen.Sections;
using Jellyfin.Plugin.TraktHomeSections.HomeScreen.Sections.Trakt;
using Jellyfin.Plugin.TraktHomeSections.HomeScreen.Sections.Latest;
using Jellyfin.Plugin.TraktHomeSections.HomeScreen.Sections.Persons;
using Jellyfin.Plugin.TraktHomeSections.HomeScreen.Sections.RecentlyAdded;
using Jellyfin.Plugin.TraktHomeSections.HomeScreen.Sections.Upcoming;
...
namespace Jellyfin.Plugin.TraktHomeSections.HomeScreen
{
    /// <summary>
    ...
    public class HomeScreenManager : IHomeScreenManager
    {
        ...
        public HomeScreenManager(IServiceProvider serviceProvider, IApplicationPaths applicationPaths, ILogger<HomeScreenManager> logger)
        {
            ...
            string userFeatureEnabledPath = Path.Combine(m_applicationPaths.PluginConfigurationsPath, typeof(HomeScreenManager).Namespace!, "userFeatureEnabled.json");
        ...
        private void RegisterRecentlyAddedSections()
        {
            RegisterResultsDelegate<RecentlyAddedMoviesSection>();
            RegisterResultsDelegate<RecentlyAddedShowsSection>();
            RegisterResultsDelegate<RecentlyAddedAlbumsSection>();
        ...
        private void RegisterLatestSections()
        {
            RegisterResultsDelegate<LatestMoviesSection>();
            RegisterResultsDelegate<LatestShowsSection>();
        }
        ...
        {
            m_delegates.Clear();
            RegisterBuiltInResultsDelegates();
        }
        ...
        {
            string pluginSettings = Path.Combine(m_applicationPaths.PluginConfigurationsPath, typeof(HomeScreenManager).Namespace!, c_settingsFile);
            ...
            IEnumerable<SectionSettings> defaultEnabledSections =
                Plugin.Instance.Configuration.SectionSettings.Where(x => x.Enabled);
            ...
            {
                settings.EnabledSections.AddRange(Plugin.Instance.Configuration.SectionSettings.Where(x => x.Enabled).Select(x => x.SectionId));
            }

            if (settings != null)
            {
                IEnumerable<SectionSettings> forcedSectionSettings = Plugin.Instance.Configuration.SectionSettings.Where(x => !x.AllowUserOverride);
