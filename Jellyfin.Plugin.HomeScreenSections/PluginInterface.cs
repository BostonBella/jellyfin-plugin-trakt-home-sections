...
namespace Jellyfin.Plugin.TraktHomeSections
{
    public static class PluginInterface
    {
        ...
        {
            IHomeScreenManager homeScreenManager = Plugin.Instance.ServiceProvider.GetRequiredService<IHomeScreenManager>();
            IServerApplicationHost serverApplicationHost = Plugin.Instance.ServiceProvider.GetRequiredService<IServerApplicationHost>();
                ...
                homeScreenManager.RegisterResultsDelegate(new PluginDefinedSection(payload.Id, payload.DisplayText!, payload.Route, payload.AdditionalData)
                {
                    OnGetResults = sectionPayload =>
                    {
                        ...
                        if (payload.ResultsAssembly != null && payload.ResultsClass != null && payload.ResultsMethod != null)
                        {
                            ...
                            {
                                object resultsHandler = ActivatorUtilities.CreateInstance(Plugin.Instance.ServiceProvider, resultsHandlerClass);
