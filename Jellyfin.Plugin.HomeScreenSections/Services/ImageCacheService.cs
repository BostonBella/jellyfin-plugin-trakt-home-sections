...
namespace Jellyfin.Plugin.TraktHomeSections.Services
{
    public class ImageCacheService
    {
        ...
        public async Task<string?> GetOrCacheImage(string sourceUrl, int cacheTimeoutSeconds)
        {
            ...
            if (m_imageCache.Count >= Plugin.Instance.Configuration.MaxImageCacheEntries)
            {
                EvictOldEntries();
            }
        ...
        private void EvictOldEntries()
        {
            List<string> oldestKeys = m_imageCache.Values
                ...
                .Take(Plugin.Instance.Configuration.MaxImageCacheEntries / 10)
                .Select(x => x.CacheKey)
                .ToList();
        ...
        private byte[] ProcessImage(byte[] imageData)
        {
            try
            {
                ...
                if (originalBitmap.Width > Plugin.Instance.Configuration.MaxImageWidth)
                {
                    SKBitmap? resizedBitmap = ResizeImage(originalBitmap, Plugin.Instance.Configuration.MaxImageWidth);
        ...
        private static byte[] CompressImage(SKBitmap bitmap)
        {
            ...
            using SKData data = image.Encode(SKEncodedImageFormat.Jpeg, Plugin.Instance.Configuration.ImageJpegQuality);
            return data.ToArray();
        }
