...
namespace Jellyfin.Plugin.TraktHomeSections.Services
{
    public class TranslationManager : ITranslationManager
    {
        ...
        public void Initialize()
        {
            ...
            m_logger.LogTrace($"Available resources: {string.Join(',', Plugin.Instance.GetType().Assembly.GetManifestResourceNames())}");
            
            // Get all the json files from the embedded resources
            string[] locJsonFiles = Plugin.Instance.GetType().Assembly.GetManifestResourceNames()
            ...
            foreach (string locFile in locJsonFiles)
            {
                ...
                using Stream? locStream = Plugin.Instance.GetType().Assembly.GetManifestResourceStream(locFile);
