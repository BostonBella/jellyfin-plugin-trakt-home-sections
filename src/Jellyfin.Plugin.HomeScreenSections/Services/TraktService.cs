...
public class TraktService
{
    ...
    private async Task RefreshAccessToken()
    {
        ...
        {
            _logger.LogWarning("Cannot refresh Trakt token because client ID or client secret is missing.");
            return;
        }
        ...
        try
        {
            ...
            var token = await response.Content.ReadFromJsonAsync<TraktAccessToken>(_jsonOptions);
            if (token == null)
            {
                return;
            }
    ...
    public async Task<TraktDeviceCode?> AuthorizeDevice()
    {
        if (string.IsNullOrWhiteSpace(TraktClientId))
        {
            _logger.LogWarning("Cannot authorize Trakt device because client ID is missing.");
            return null;
        }
        ...
        return await response.Content.ReadFromJsonAsync<TraktDeviceCode>(_jsonOptions);
    }
    ...
    public async Task<bool> PollForToken(TraktDeviceCode deviceCode)
    {
        ...
        if (string.IsNullOrWhiteSpace(TraktClientId) || string.IsNullOrWhiteSpace(TraktClientSecret))
        {
            _logger.LogWarning("Cannot poll for Trakt token because client ID or client secret is missing.");
            return false;
        }
        ...
        while (DateTime.UtcNow < expiresAt)
        {
            ...
            if (response.StatusCode == HttpStatusCode.OK)
            {
                ...
                _logger.LogInformation("Successfully authorized Trakt account.");
                return true;
            }
    ...
    public async Task<TraktListItem[]> GetListItems(string listId, int limit = 20)
    {
        ...
        {
            _logger.LogError("Failed to get Trakt list items for list {ListId}: {Status}", listId, response.StatusCode);
            return Array.Empty<TraktListItem>();
        }

        var items = await response.Content.ReadFromJsonAsync<TraktListItem[]>(_jsonOptions);
    ...
    public async Task<TraktListItem[]> GetWatchlist(int limit = 20)
    {
        ...
        {
            var movies = await moviesResponse.Content.ReadFromJsonAsync<TraktWatchlistMovieItem[]>(_jsonOptions);
            if (movies != null)
            {
        ...
        {
            var shows = await showsResponse.Content.ReadFromJsonAsync<TraktWatchlistShowItem[]>(_jsonOptions);
            if (shows != null)
            {
    ...
    public async Task<TraktMovie[]> GetMovieRecommendations(int limit = 20)
    {
        ...
        if (!response.IsSuccessStatusCode)
        {
            _logger.LogError("Failed to get Trakt movie recommendations: {Status}", response.StatusCode);
    ...
    public async Task<TraktShow[]> GetShowRecommendations(int limit = 20)
    {
        ...
        if (!response.IsSuccessStatusCode)
        {
            _logger.LogError("Failed to get Trakt show recommendations: {Status}", response.StatusCode);
    ...
    public async Task<TraktMovie[]> GetTrendingMovies(int limit = 20)
    {
        ...
        if (!response.IsSuccessStatusCode)
        {
            _logger.LogError("Failed to get Trakt trending movies: {Status}", response.StatusCode);
            return Array.Empty<TraktMovie>();
        }
